...
def _row_public(p: Publisher) -> dict:
    ...
    return {"id": p.publisher_id, "name": p.name, "type": p.type, "kind": p.kind,
...
def list_publishers(kind: Optional[str] = None, *, runtime: bool = False) -> List[dict]:
...
def get_publisher(publisher_id: str, *, runtime: bool = False) -> Optional[dict]:
    with get_session() as s:
...
def create_publisher(*, name: str, type: str, config: dict, kind: str = "favorite",
    ...
    enc = config_secrets.encrypt_config(config or {})
    ...
    with get_session() as s:
        p = Publisher(publisher_id=publisher_id or str(uuid.uuid4()), name=name, type=type, kind=kind,
                      enabled=bool(enabled), config=enc, created_by=created_by)
        s.add(p); s.flush()
        return _row_public(p)


def update_publisher(publisher_id: str, *, name=None, description=None, type=None,
                     config: Optional[dict] = None, enabled=None) -> Optional[dict]:
...
def delete_publisher(publisher_id: str) -> bool:
    with get_session() as s:
        p = s.execute(select(Publisher).where(Publisher.publisher_id == publisher_id)).scalar_one_or_none()
        if p is None:
            return False

...
class HardwareDetector():
    ...
    def _detect_intel_hardware(self) -> Dict[str, Any]:
        ...
        try:
            # OpenVINO 2026.x REMOVED the openvino.runtime module; Core moved to the
            # package root. Try the modern path first and keep the legacy one for
            # older installs, otherwise this silently falls through to the legacy
            # probe below - which shells out to `lspci`, absent from the container -
            # and the node reports a device list it cannot actually execute on.
            try:
    ...
    def optimize_device_string(self, device: str, hardware_info: Optional[Dict[str, Any]] = None) -> str:
        ...
        # 'CPU' means the CPU. It is NOT silently upgraded to an Intel/OpenVINO runtime -
        # asking for cpu and being given OpenVINO made the effective device unpredictable
        # and triggered the OpenVINO export path. Explicit 'intel:cpu' still selects it.
        if device_base == 'CPU':
            return f"cpu{device_index}".lower()
        
        elif device_base == 'GPU':
            # CUDA first, and trust torch over string-scraped vendor detection: a failed
            # nvidia-smi/PowerShell probe must not silently route an NVIDIA box to Intel.
            if hardware_info['nvidia']['gpu'] or self._torch_cuda_available():
    ...
    def _torch_cuda_available() -> bool:
        """Second, independent signal for CUDA presence. `_detect_nvidia_hardware` parses
        nvidia-smi/lspci/PowerShell output, which can fail while CUDA works perfectly."""
        try:
            import torch

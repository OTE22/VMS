...
class InferenceEngineFactory:
    ...
    def _discover_engines(cls):
        ...
        # ---- registry-gated CUSTOM engines under ARTIFACT_ROOT/engines/<key>/engine.py
        # Only rows the registry deems servable (AVAILABLE + PASSED + present + sha256
        # re-verified NOW) are imported - a STAGING/FAILED/MISSING/CORRUPT/DELETING engine is
        # never activated. Builtins discovered above are recorded in the registry so the
        # two origins are explicit and never mixed.
        cls._discover_custom_engines_from_registry()

        cls._discovery_complete = True
        logger.info(f"Engine discovery complete. Found {len(cls._engine_types)} engines.")

    @classmethod
    def _discover_custom_engines_from_registry(cls):
        try:
            from InferenceNode import engine_registry as reg
            from InferenceNode.auth import db as _db
            if _db._engine is None:
                return                      # no DB configured (e.g. import-time tools): builtins only
            for key, obj in list(cls._engine_types.items()):
                try:
                    reg.register_builtin(key, obj.__name__, cls._engine_display_names.get(key),
                                         source_path=getattr(sys.modules.get(obj.__module__), "__file__", None))
                except Exception as e:  # noqa: BLE001
                    logger.debug(f"builtin registry record for {key}: {e}")
            for e in reg.list_engines(origin="custom"):
                key = e["engine_key"]
                path = reg.servable_custom_path(key)      # hash-verified on load
                if not path:
                    logger.warning(f"[ENGINES] custom engine {key} not servable "
                                   f"(status={e['status']} validation={e['validation_status']}) - not loaded")
                    continue
                try:
                    import importlib.util
                    # A generated engine falls back to `from base_engine import BaseInferenceEngine`
                    # (it lives outside the package). Alias that name to the SAME module object
                    # the factory uses, so issubclass() sees one BaseInferenceEngine class - not
                    # a second copy imported from the source tree via sys.path.
                    base_mod = sys.modules.get(BaseInferenceEngine.__module__)
                    if base_mod is not None and sys.modules.get("base_engine") is not base_mod:
                        sys.modules["base_engine"] = base_mod
                    spec = importlib.util.spec_from_file_location(f"armyeye_custom_engine_{key}", path)
                    module = importlib.util.module_from_spec(spec)
                    sys.modules[spec.name] = module
                    spec.loader.exec_module(module)
                    for name, obj in inspect.getmembers(module, inspect.isclass):
                        if (obj != BaseInferenceEngine and issubclass(obj, BaseInferenceEngine)
                                and obj.__module__ == module.__name__):
                            ekey = cls._class_name_to_key(name)
                            if ekey == key and ekey not in cls._engine_types:
                                cls._engine_types[ekey] = obj
                                cls._engine_display_names[ekey] = getattr(obj, "display_name",
                                                                          ekey.replace("_", " ").title())
                                logger.info(f"Loaded custom engine from registry: {ekey} -> {obj.__name__}")
                except Exception as ex:  # noqa: BLE001
                    logger.warning(f"[ENGINES] failed to import custom engine {key}: {ex}")
        except Exception as e:  # noqa: BLE001 - registry unavailable must never break builtin discovery
            logger.debug(f"custom engine discovery skipped: {e}")
